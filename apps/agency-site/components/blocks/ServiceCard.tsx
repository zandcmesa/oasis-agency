...
export function ServiceCard({ service }: { service: Service }) {
  const { number, title, copy, image, alt, frame, href, example, tags } = service;
  return (
    <article className="snap-start shrink-0 w-[85vw] lg:w-[min(85vw,1120px)] lg:h-[68vh] lg:min-h-[540px] rounded-lg overflow-hidden bg-ink text-paper grid lg:grid-cols-2">
      ...
      {image && frame === "browser" ? (
        <div className="order-1 lg:order-2 relative bg-oasis-green aspect-[4/3] lg:aspect-auto lg:h-full overflow-hidden p-7 lg:p-12 flex items-center justify-center">
          <span className="absolute -right-[0.05em] -bottom-[0.18em] text-wordmark font-sans text-paper/12 select-none" aria-hidden="true">
            {number}
          </span>
          <BrowserFrame src={withBasePath(image)} alt={alt} className="relative w-full max-w-[440px]" />
        </div>
      ) : image ? (
